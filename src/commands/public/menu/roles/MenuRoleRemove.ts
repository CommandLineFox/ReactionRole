...
export default class MenuRoleRemove extends Subcommand {
    public constructor() {
        super("remove", "Remove a role from a specified role menu");
    ...
    async execute(interaction: CommandInteraction, client: BotClient): Promise<void> {
        ...
        await client.database.guilds.updateOne({ id: guild.id, "menus.name": menu.name }, { "$pull": { "menus.$.roles": menuRole } });
        await interaction.reply(`Successfully removed ${role.name} from ${name}'s role list.`);
    }
}
